...
下面是当前已完成的中文口径本地实测面板，使用 no-thinking native harness；总分为 deterministic partial 加权分，满分 100；通过项为 strict `x/60`。维度列为各维度 partial 子分，每项满分 10；这些结果只代表中文测试口径，不能和 `en-US` 国际口径混榜。后续正式公开榜单应按 `0.2.0 zh-CN` manifest 重新归档或复算。
